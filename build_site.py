...
/* "Fela kort" — small × at the top-right corner of the map */
.map-hide-toggle {
    position: absolute;
    top: 12px;
    right: 12px;
...
.map-hide-toggle:active { transform: scale(0.94); }

/* "Sýna kort" — appears in the layout's top-right when map is hidden */
.map-show-toggle {
    position: absolute;
    top: 0;
    right: 0;
...
/* When map is hidden, the journal can breathe wider — but cap so the list
   doesn't sprawl edge-to-edge on huge screens. */
.dagbok-layout.map-hidden .journal-pane {
    max-width: 1080px;
    margin: 0 auto;
}
...
/* On desktop, hang-indent the body so it starts where the title does in
   the summary above (110px date column + 18px gap = 128px), and cap the
   reading width at 760px so prose stays comfortable. */
...
@media (max-width: 900px) {
    ...
    .map-show-toggle { position: static; margin: 0 0 20px; align-self: flex-start; }
    .dagbok-layout.map-hidden { display: flex; flex-direction: column; }
}
